...
pub async fn upload_single(opener: &mut reqwest::Client, src: Vec<u8>) -> Result<String> {
...
pub async fn upload_batch(
    ...
    let stream = futures::stream::iter((0..epoch).map(|epoch_id| {
...
pub async fn get_index(url: &str, key: &[u8]) -> Result<(u8, String, u64, Vec<String>)> {
...
pub async fn download_batch(
    ...
    let mut stream = futures::stream::iter(urls.iter().enumerate().map(async move |(idx, x)| {
